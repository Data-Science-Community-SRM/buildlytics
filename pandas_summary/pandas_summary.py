...
class DataFrameSummary(object):
    ...
    def _get_columns_info(self, stats):
        ...
        column_info[self.TYPE_NUMERIC] = pd.Index([c for c in rest_columns if types.is_numeric_dtype(self.df[c])])
    ...
    def _get_top_correlations(self, column, threshold=0.65, top=3):
        column_corr = np.fabs(self.corr[column].drop(column)).sort_values(ascending=False,inplace=False)
    ...
    def _get_numeric_summary(self, column):
        ...
        return pd.concat([pd.Series(stats, name=column),self.columns_stats[column]],sort=True)
